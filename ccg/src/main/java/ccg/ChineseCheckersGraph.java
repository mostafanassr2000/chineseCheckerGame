	...
	public ChineseCheckersGraph() {
		...
		for (int i = 0; i < coordMat.length; i++) {
			for (int j = 0; j < coordMat[i].length; j++) {
				counter++;
				System.out.println(coordMat[i][j]);
			}
...
/* 
		for (int i = 0; i < coordMat.length; i++) {
			for (int j = 0; j < coordMat[i].length; j++) {
				if (GraphFacilities.vertexMat[coordMat[i][j].y][coordMat[i][j].x] == null) {
					if (coordMat[i][j].y < 4) {
		...
		for (int i = 0; i < logicMat.length; i++) {
			for (int j = 0; j < logicMat[i].length; j++) {
				if (GraphFacilities.vertexMat[i][j] == null) {
					if (logicMat[i][j] == 2) {
						GraphFacilities.vertexMat[i][j] = new CellVertex(new Point(j, i), PlayerEnum.COMPUTER);
					} else if (logicMat[i][j] == 1) {
						GraphFacilities.vertexMat[i][j] = new CellVertex(new Point(j, i), PlayerEnum.PLAYER);
					} else {
						GraphFacilities.vertexMat[i][j] = new CellVertex(new Point(j, i));
					}
				}
			}
		}
	...
	public Set<Point> optinalPlays(int x, int y) {
		...
		while (it.hasNext()) {
			...
			if (!edge.getSrcVertx().getLocation().equals(new Point(x, y))) {
				// Cell empty
				if (edge.getSrcVertx().content != PlayerEnum.CPU_PLAYER
						&& edge.getSrcVertx().content != PlayerEnum.HUMAN_PLAYER) {
